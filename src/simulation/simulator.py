...
Usage from CLI:
    python simulator.py --disease "Tomato___Septoria_leaf_spot" --days 30
"""
...
from spread_model import SpreadModel, DEFAULT_BETA, DEFAULT_GAMMA
from interventions import INTERVENTION_REGISTRY, BaseIntervention
...
# Values derived from agricultural literature approximations
DISEASE_BETA_OVERRIDES: dict[str, float] = {
    "Tomato___Late_blight":              0.45,   # Very aggressive
    "Tomato___Early_blight":             0.30,
    "Tomato___Septoria_leaf_spot":       0.28,
    "Corn___Northern_Leaf_Blight":       0.35,
    "Potato___Late_blight":              0.50,   # Most aggressive
    "Apple___Apple_scab":                0.25,
    "Grape___Black_rot":                 0.32,
}
...
def _run_single_scenario(
    ...
    Returns:
        Dictionary with keys:
            ...
            'description':       str
            'daily_counts':      list of dicts (one per day)
            'final_summary':     dict with final state counts
            'peak_infected':     int — maximum infected cells in any day
            'total_crop_loss':   int — final Removed + Infected cell count
            'spread_contained':  bool — True if infected count is 0 on final day
    """
...
def run_simulation(
    ...
    Returns:
        Dictionary with keys:
            ...
            'grid_size':     [rows, cols]
            'num_days':      int
            'scenarios':     list of scenario result dicts (one per intervention)
            'ranked_by_crop_loss': list of intervention names sorted best to worst
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run pest spread simulation for a detected disease.")
    parser.add_argument(
        "--disease", type=str, default="Tomato___Septoria_leaf_spot",
        help="PlantVillage disease class name (from Lab 5 model output)."
    )
    parser.add_argument(
        "--days", type=int, default=DEFAULT_SIMULATION_DAYS,
        help="Number of days to simulate."
    )
    parser.add_argument(
        "--rows", type=int, default=DEFAULT_GRID_ROWS,
        help="Number of rows in the farm grid."
    )
    parser.add_argument(
        "--cols", type=int, default=DEFAULT_GRID_COLS,
        help="Number of columns in the farm grid."
    )
    ...
    print(f"\nBest strategy: {results['best_strategy']} "
          f"({results['best_crop_loss_percent']}% crop loss)")

    # Pretty-print JSON for inspection (trim daily_counts for readability)
    compact = {k: v for k, v in results.items() if k != "scenarios"}
    print("\nSummary JSON:")
